...
for resultsFolder in resultsFolders:
    ...
    plot_pdf(pos_pdf, neg_pdf, pdfax, resultsFolder, colors[color])
    pdfax.hist(pos_predicts, bins=25, alpha=0.6, color="c")
    pdfax.hist(neg_predicts, bins=25, alpha=0.6, color=colors[color])
    color+=1
