...
folders = [config.myStudies+"/"+folder+"/contigs/"+config.selectedTask+"_"+str(config.contigLength) for folder in os.listdir(config.myStudies) if "ref" in folder]
paths_to_train = []
...
for path in paths_to_train:
    ...
    data, indeces, labels = convnet.load_numpy_stack_age(path, paths, permuteLabels=config.wumbo)
...
fitted, modelvar = convnet.createAgeModel(train_data, train_labels, test_data, test_labels, rate, epochs, beta1, beta2)
